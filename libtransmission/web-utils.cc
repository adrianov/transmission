...
{
    static auto constexpr MaxLen = size_t{ 16384 };
...
[[nodiscard]] std::string httpErrorBodyExcerpt(std::string_view body)
{
    ...
    auto plain = collapseWs(stripTags(work));
    if (std::empty(plain))
    {
        return {};
    }
...
std::string tr_webFormatTrackerHttpError(long const status, std::string_view const body)
{
    auto const* const label = tr_webGetResponseStr(status);
    auto const excerpt = httpErrorBodyExcerpt(body);

    if (std::empty(excerpt))
    {
        return fmt::format("Tracker HTTP response {:d} ({:s})", status, label);
    }

    if (std::string_view{ label } == "Unknown Error"sv)
    {
        return fmt::format("Tracker HTTP response {:d} - {:s}", status, excerpt);
    }

    return fmt::format("Tracker HTTP response {:d} ({:s}) - {:s}", status, label, excerpt);
}
