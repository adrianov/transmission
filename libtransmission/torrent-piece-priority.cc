...
void tr_torrent::recalculate_file_order()
{
    ...
    // Sort wanted files by path using natural order (S01E00 before S01E01, S01E2 before S01E10).
    std::sort(
        wanted_files.begin(),
        wanted_files.end(),
        [this](tr_file_index_t a, tr_file_index_t b)
        {
            return tr_compare_paths_for_piece_priority(files().path(a), files().path(b)) < 0;
        });
